...
Use this [applet](https://www.rossmanchance.com/applets/2021/oneprop/OneProp.htm) to conduct the simulation.
...
For example, if $\theta = 0.1$ we wouldn't expect to see a sample with $y > 9$; if $\theta = 0.9$ we wouldn't expect to see a sample with $y < 16$.
...
We simulated values of $y$ for each of the $\theta$ values, but the values of $\theta$ were not equally plausible.
...
1. Conduct a few repetitions of this simulation and enter your results in this Google sheet.
**NEED TO CREATE GOOGLE SHEET WHERE PARTICIPANTS CAN ENTER SIMULATION RESULTS.**
(*With students, we would conduct some tactile simulations first.*)


1. There are two sources of uncertainty/variability in such a simulation.
What are they?
...
1. Use the plots for the previous section to (roughly) rank 0.1, 0.3, 0.5, 0.7, 0.9 from most to least plausible as values of $\theta$.
